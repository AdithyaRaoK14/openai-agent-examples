...
def run_agent_with_tool(cl):
    prompt = """
    Check internet connectivity by pinging a variety of servers.
    Identify which servers are reachable.
    Use multiple different servers of your choice.
    Produce output in formatted JSON.
    The JSON should include only three fields: 'result', 'reachable', and 'summary' in that order.
    The 'result' field stores a boolean indicating overall success.
    The 'reachable' field should list the servers that responded successfully.
    The 'summary' field should contain a brief description of the connectivity status.
    """
